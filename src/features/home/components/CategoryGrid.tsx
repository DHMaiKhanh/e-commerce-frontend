...
export function CategoryGrid() {
  return (
    <section className="rounded-2xl bg-white p-4 shadow-soft sm:p-5">
        ...
        {homeCategories.map((cat) => (
          <li key={cat.id}>
            <Link
              ...
              className="group flex flex-col items-center gap-2 rounded-2xl px-2 py-3 text-center transition-all hover:-translate-y-0.5 hover:bg-brand-50"
            >
              <span className="overflow-hidden rounded-2xl ring-1 ring-gray-100 transition group-hover:ring-2 group-hover:ring-brand-200">
                <img
                  src={cat.iconUrl}
                  alt={cat.name}
                  loading="lazy"
